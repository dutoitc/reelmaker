...
from .utils import ensure_tool, fail, probe_media_duration, read_json, write_json
...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default="hybrid",
        help="hybrid allows Ollama to assemble 2-3 complementary source passages into one reel.",
    )
    ...
    parser.add_argument("--timing-history", type=Path, help=argparse.SUPPRESS)
    parser.add_argument("--no-timing-history", action="store_true", help=argparse.SUPPRESS)
...
def load_transcript(
    ...
    source_duration = probe_media_duration(args.source_video) if args.source_video else 0.0
    if reporter:
        reporter.start_stage(
            "transcription",
            "Transcription et alignement WhisperX",
            total=max(1.0, source_duration),
            history_key=f"transcription:{provider.name}:{args.whisper_model}:{args.whisper_device}:{args.whisper_compute_type}",
            history_units=max(1.0, source_duration),
        )
    try:
    ...
    if reporter:
        reporter.finish_stage(record_history=not reused, message=cache_text)
    return document
...
def load_scenes_for_render(
    ...
    if args.crop_mode != "scene-smart" or args.scene_detection == "off":
        return None
    duration = probe_media_duration(source_video)
    if reporter:
        reporter.start_stage(
            "scenes",
            "Detection des plans",
            total=max(1.0, duration),
            history_key="scenes:pyscenedetect",
            history_units=max(1.0, duration),
        )
    try:
...
def run_transcribe(args: argparse.Namespace, reporter: ProgressReporter | None = None) -> None:
    paths = ProjectPaths.create(args.output_dir)
    load_transcript(args, paths, reporter)
    print(f"Transcript files: {paths.output_dir / 'transcript.json'} and {paths.output_dir / 'transcript.srt'}")


def run_analyze(args: argparse.Namespace, reporter: ProgressReporter | None = None) -> list[ReelSelection]:
    paths = ProjectPaths.create(args.output_dir)
    ...
    for idx, item in enumerate(raw, start=1):
        ...
        for raw_segment in item.get("segments") or []:
            if not isinstance(raw_segment, dict):
                continue
            segment_start = _parse_seconds(raw_segment.get("start"))
            segment_end = _parse_seconds(raw_segment.get("end"))
            if segment_end > segment_start:
                segments.append(ReelSegment(segment_start, segment_end))
        selections.append(
            ReelSelection(
...
def run_render(
    ...
    total_render_seconds = sum(selection.duration for selection in selections)
    if reporter:
        reporter.start_stage(
            "render",
            "Rendu des reels",
            total=max(1, len(selections)),
            history_key=f"render:{args.crop_mode}:{args.preset}:{args.fps}",
            history_units=max(1.0, total_render_seconds),
        )
    ...
    report_path = paths.output_dir / "reels" / "render_report.json"
    print(f"Rendered {ok_count}/{len(results)} reels. Report: {report_path}")
    if ok_count == 0:
...
def run_all(args: argparse.Namespace, reporter: ProgressReporter | None = None) -> None:
    selections = run_analyze(args, reporter)
    if not args.no_render:
        run_render(args, selections=selections, reporter=reporter)


def validate_runtime(args: argparse.Namespace) -> None:
    """Fail before expensive transcription when a render dependency is missing."""
    if args.command in {"all", "render", "scenes"} and not getattr(args, "no_render", False):
        try:
            ...
            fail(str(exc))
    if (
        args.command in {"all", "render", "scenes"}
        and args.crop_mode == "scene-smart"
        and args.scene_detection != "off"
    ):
        try:
            import scenedetect  # noqa: F401
        except Exception:
            fail('PySceneDetect is required for --crop-mode scene-smart. Run: pip install -e ".[vision]"')
...
def main(argv: list[str] | None = None) -> None:
    ...
    reporter = ProgressReporter(
        ...
        history_path=args.timing_history,
        use_history=not args.no_timing_history,
    )
    try:
        validate_runtime(args)
        if args.command in {"transcribe", "subtitles"}:
            run_transcribe(args, reporter)
            ...
            parser.print_help()
        reporter.finish_run(success=True)
    except SystemExit as exc:
        if exc.code not in (None, 0):
            reporter.finish_run(success=False, message=str(exc))
        raise
